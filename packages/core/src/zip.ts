...
function buildReadme(m: Manifest, failures: Array<{ url: string; error: string }>): string {
  ...
  const lines = [
    ...
    `由 [Matters 記憶吐司](https://github.com/thematters/matters-lifeboat) 於 ${m.exportedAt} 產生。`,
    ``,
    `## 內容`,
    ``,
    ...
    `- 結構：`,
    `  - \`posts/<日期>-<shortHash>.md\` — 每一篇文章（Markdown + YAML frontmatter；標題與原始 slug 在 frontmatter 裡）`,
    `  - \`assets/\` — 所有圖片`,
    ...
    ``,
    `**要先做最輕量保存，可用 Matters 記憶吐司的「文章地址簿」流程產生一張 IPFS 連結清單。`,
    `要多放一份到線上保存空間，則用 Pinata 流程，把可分享的文章地址頁和完整備份 ZIP 一起上傳到你自己的 Pinata public IPFS。**`,
    ``,
    `## 做成自己的網站`,
    ``,
    `最簡單的方式：把本 zip 上傳到 [Matters 記憶吐司架站流程](https://lifeboat.matters.town/app/?flow=c)，`,
    `產出純 HTML/CSS 的 \`site.zip\`。解壓縮後把整個資料夾拖到 Cloudflare Workers & Pages 的 Upload your static files 即可，`,
    `不需要 GitHub、不需要 \`npm install\`、不需要 build command。`,
    ``,
    `若你想自行客製，可以把 \`posts/\` 與 \`assets/\` 交給熟悉網站製作的人處理。`,
    ``,
  ];
...
function buildAgentInstructions(m: Manifest): string {
  return [
    ...
    `- \`posts/<YYYY-MM-DD>-<shortHash>.md\`：每篇文章，含 YAML frontmatter（title / slug / createdAt / tags / ipfs.dataHash 等）`,
    ...
    `## 最短部署路徑（推薦給使用者）`,
    ``,
    `1. 開啟 Matters 記憶吐司 C 流程：\`/app/?flow=c\``,
    `2. 上傳這份 \`backup.zip\`，產出純靜態的 \`site.zip\``,
    `3. 解開 \`site.zip\`，確認根目錄有 \`index.html\`、\`style.css\`、\`posts/\`、\`assets/\``,
    `4. 到 Cloudflare Workers & Pages 的 Upload your static files，把整個解開後的資料夾拖上去；不需要 build command`,
    `5. 若使用者有 Pinata JWT，先產生文章地址頁並上傳為 public IPFS file，再上傳完整 \`backup.zip\`；若帳號支援 \`pinByHash\`，才進階批次 pin \`MANIFEST.json.articles[*].dataHash\``,
    ``,
    `## 給使用者的白話說明（你可以複述）`,
    ``,
    `- 這份 zip 是你的 matters 文章的完整備份，**無需任何雲端服務**即可本機瀏覽（用任意 Markdown 閱讀器開 \`posts/\` 裡任一檔）`,
    `- 若你想做一個屬於自己的網站，告訴我「請幫我用這份備份部署到 Cloudflare Pages」`,
    `- 若你只想先保存文章地址，告訴我「請幫我產生 文章地址簿」`,
    `- 若你想讓這些文章在 IPFS 上更好分享與保存，告訴我「請幫我把文章地址頁和 backup.zip 上傳到 Pinata」`,
    ``,
    `## 備援：純手動路徑`,
    ``,
    `任何 SSG 都能吃這個格式。frontmatter 是標準 YAML，markdown 是 GitHub-flavored。`,
    `若只是要閱讀，直接開 \`posts/*.md\` 用任何 Markdown 閱讀器讀即可。`,
    ``,
    `—— Matters 記憶吐司（${m.exportedAt}）`,
    ``,
  ].join("\n");
}
