...
function buildReadme(m: Manifest, failures: Array<{ url: string; error: string }>): string {
  ...
  const lines = [
    ...
    `由 [Matters 救生艇](https://github.com/thematters/matters-lifeboat) 於 ${m.exportedAt} 產生。`,
    ``,
    `## 內容`,
    ``,
    ...
    ``,
    `**要先做最輕量保存，可用 Matters 救生艇的「文章地址簿」流程產生一張 IPFS 連結清單。`,
    `要多放一份到線上保存空間，則用 Pinata 流程，把可分享的文章地址頁和完整備份 ZIP 一起上傳到你自己的 Pinata public IPFS。**`,
    ``,
    `## 做成自己的網站`,
    ``,
    `最簡單的方式：把本 zip 上傳到 [Matters 救生艇架站流程](https://lifeboat.matters.town/app/?flow=c)，`,
...
function buildAgentInstructions(m: Manifest): string {
  return [
    ...
    `## 最短部署路徑（推薦給使用者）`,
    ``,
    `1. 開啟 Matters 救生艇 C 流程：\`/app/?flow=c\``,
