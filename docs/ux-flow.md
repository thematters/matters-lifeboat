# Matters 記憶吐司 (matters-lifeboat) — UX 流程規格
...
| 視覺重點 | 一個半透明「記憶吐司」SVG 漂浮在海面上，下方是排列成浪的文章卡片剪影；**不畫沈船**。 | Same. |
...
- **文章地址在你手上**：你的每一篇文章都有一個內容地址（CID），這是去中心化的好處；記憶吐司幫你把地址整理成一份可攜的檔案。
...
- **Loading**：
  ...
  - 預估剩餘時間（檔案大時要老實告知）
  - 可最小化到 browser tab title：`(47/143) Matters 記憶吐司`
...
- 不會自動同步新文章。下次你發新文章，想更新就再跑一次記憶吐司。
...
（MVP 階段）：回來跑一次記憶吐司，會幫你 push 新文章到同一個 repo，網站自動重新 build。
...
- GitHub: Settings → Applications → Authorized OAuth Apps → Matters 記憶吐司 → Revoke
